...
class Calculator extends React.Component {
	...
	signChange() {
		this.setState((state)=>{
			let obj = state;
			if( new RegExp(`^\\-?${regexNumber(1)}${regexOper()}${regexNumber(1)}$`).test(obj.value)) {
				let [,left,right] = obj.value.match(new RegExp(`^(\\-?${regexNumber(1)}${regexOper()})(${regexNumber(1)})$`));
				obj.value = `${left}(-${right}`;
			}else if(new RegExp(`${regexOper()}\\(\\-${regexNumber(1)}$`).test(obj.value)) {
				let [,left,right] = obj.value.match(new RegExp(`^(\\-?${regexNumber(1)}${regexOper()})(\\(\\-${regexNumber(1)})$`));
	...
	handleChange(event) {
		...
		if(event.keyCode===8 || new RegExp(regexNumber()).test(char) || new RegExp(regexOper(true)).test(char))
			this.setState((state)=>{
				...
				if(!(new RegExp(`^\\-?${regexNumber(0)}${regexOper()}?\\(?\\-?${regexNumber(0)}$`).test(obj.value))) obj.value = '';
				...
				if((!obj.value.length && char==='-') || (obj.value==='-' && char==='+')) obj.value = obj.value.length?'': '-';
					...
					obj.value = obj.value.replace(new RegExp(`${regexOper()}$`),'');
					if( new RegExp(`^\\-?${regexNumber(1)}${regexOper()}\\(?\\-?${regexNumber(1)}$`).test(obj.value)) {
						obj.b = Number(obj.value.match(new RegExp(`(${regexOper()}\\(?)(\\-?${regexNumber(1)}$)`))[2]);
					...
					obj.value += char;
					if (/[^\.]/.test(char) && (new RegExp(`^0${regexNumber(1)}`).test(obj.value) || new RegExp(`${regexOper()}\\(?\\-?0${regexNumber(1)}$`).test(obj.value)))
						obj.value = obj.value.replace(new RegExp(`^(\\-)?(${regexNumber(1)})(${regexOper()}\\(?\\-?)?(${regexNumber(1)}$)?`),function(all,sign='',a,oper='',b='') {
